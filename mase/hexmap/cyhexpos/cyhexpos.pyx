...
cdef class CyHexPos:
    ...
    def pathfind_dfs(self, target, avoidset: set = None, max_dist: int = None, verbose: bool = False):
        ...
        visited = set([self])
        current_path: List[CyHexPos] = [self]
    ...
    cdef CyHexPos cy_offset(self, int delta_q, int delta_r, int delta_s):
        return CyHexPos(self.q+delta_q, self.r+delta_r, self.s+delta_s)

    cpdef float cy_dist(self, CyHexPos other):
